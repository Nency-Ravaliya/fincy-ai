...
export function Chat({ onProfileClick }: { onProfileClick: () => void }) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // Check message count and subscription status
    ...
    if (user) {
      ...
      if (messageCount >= 10 && !isSubscribed) {
        alert('Please subscribe to continue using the chat');
        return;
      }
  ...
  return (
    <div className="flex h-screen bg-[#1A1D1E] overflow-hidden">
      ...
      {/* Desktop Header */}
      <div className="hidden md:flex fixed top-0 right-0 h-16 bg-[#141718] z-40 items-center px-4 border-b border-gray-800">
        <SubscriptionButton />
      </div>
